  ...
  def _build_panels(self):
    ...
    self._advanced_rows = [
      ...
      SettingRow(
        ...
        visible=lambda: alt_on() and cs.isAccord9G,
      ),
      SettingRow(
        ...
        visible=lambda: alt_on() and cs.isAccord9G and p.get_bool("TorqueInterceptorEnabled"),
      ),
      SettingRow(
      ...
      SettingRow(
        ...
        get_value=lambda: f"{p.get_float('TISigmoidA'):.1f}",
        on_click=lambda: self._show_slider("TISigmoidA", 1.0, 15.0, step=0.1, value_type="float"),
        visible=lambda: alt_on() and cs.isAccord9G and p.get_bool("TorqueInterceptorEnabled") and p.get_bool("TISigmoidEnabled"),
      ),
      SettingRow(
        "TISigmoidB", "value", tr_noop("Sigmoid Scale (b)"),
        subtitle=tr_noop("Overall nonlinear magnitude."),
        get_value=lambda: f"{p.get_float('TISigmoidB'):.2f}",
      ...
      SettingRow(
        ...
        get_value=lambda: f"{p.get_float('TISigmoidC'):.2f}",
      ...
      SettingRow(
        ...
        get_value=lambda: f"{p.get_float('TISteerKp'):.2f}",
